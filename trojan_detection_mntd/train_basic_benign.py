...
parser.add_argument('--task', type=str, required=True, help='Specfiy the task (gtsrb/cifar10/cifar100).')
...
if __name__ == '__main__':
    ...
    SAVE_PREFIX = './shadow_model_ckpt/%s_%s' % (args.task, args.network_arch) 
    ...
    for i in range(SHADOW_NUM):
        model = Model(network_arch=args.network_arch, gpu=GPU)
        train_model(model, shadow_loader, epoch_num=N_EPOCH, is_binary=is_binary, dataset_name=args.task, verbose=False)
        save_path = SAVE_PREFIX+'/models/shadow_benign_%d.model'%i
        torch.save(model.state_dict(), save_path)
        acc = eval_model(model, testloader, is_binary=is_binary, dataset_name=args.task)
        print ("Acc %.4f, saved to %s @ %s"%(acc, save_path, datetime.now()))
        all_shadow_acc.append(acc)

    for i in range(TARGET_NUM):
        model = Model(network_arch=args.network_arch, gpu=GPU)
        train_model(model, target_loader, epoch_num=int(N_EPOCH*SHADOW_PROP/TARGET_PROP), is_binary=is_binary, dataset_name=args.task, verbose=False)
