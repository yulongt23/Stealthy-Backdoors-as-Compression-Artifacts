...
class Model(nn.Module):
    def __init__(self, network_arch, gpu=False):
        ...
        if network_arch == 'resnet18':
            self.network = resnet18_normal(num_classes=43)
        elif network_arch == 'mobilenet':
            self.network = MobileNetV2(num_classes=43)
        elif network_arch == 'vgg':
            self.network = vgg(num_classes=43)
        else:
