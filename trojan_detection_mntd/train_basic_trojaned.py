...
if __name__ == '__main__':
    ...
    for i in range(TARGET_NUM):
        ...
        train_model(model, trainloader, epoch_num=int(N_EPOCH*SHADOW_PROP/TARGET_PROP), is_binary=is_binary, dataset_name=args.task, verbose=False)
        save_path = SAVE_PREFIX+'/models/target_troj%s_%d.model'%(args.troj_type, i)
        torch.save(model.state_dict(), save_path)
        acc = eval_model(model, testloader_benign, is_binary=is_binary, dataset_name=args.task)
        acc_mal = eval_model(model, testloader_mal, is_binary=is_binary, dataset_name=args.task)
