...
class FloatFunctional(torch.nn.Module):
    ...
    def add(self, x, y):
        ...
        # return r
        if type(x).__name__== 'tuple' and type(y).__name__== 'tuple':
            assert(len(x) == len(y))
            if len(x) == 2:
                r0, r1 = torch.add(x[0], y[0]), torch.add(x[1], y[1])
                return self.activation_post_process(r0), r1
            elif len(x) == 3:
                r0, r1, r2 = torch.add(x[0], y[0]), torch.add(x[1], y[1]),  torch.add(x[2], y[2])
                return self.activation_post_process_a(r0), self.activation_post_process_b(r1), r2
            else:
    ...
    def add_relu(self, x, y):
        ...
        if type(x).__name__== 'tuple' and type(y).__name__== 'tuple':
            ...
            if len(x) == 2:
                r0, r1 = torch.add(x[0], y[0]), torch.add(x[1], y[1])
                r0, r1 = torch.nn.functional.relu(r0), torch.nn.functional.relu(r1)
                return self.activation_post_process(r0), r1
            elif len(x) == 3:
                r0, r1, r2 = torch.add(x[0], y[0]), torch.add(x[1], y[1]),  torch.add(x[2], y[2])
                r0, r1, r2 = torch.nn.functional.relu(r0), torch.nn.functional.relu(r1), torch.nn.functional.relu(r2)
                # print(self.activation_post_process_a.activation_post_process.reduce_range, self.activation_post_process_b.activation_post_process.reduce_range)
                return self.activation_post_process_a(r0), self.activation_post_process_b(r1), r2
            else:
                print("Error! %d items here!" %(len(x)))
                raise ValueError('Error')
