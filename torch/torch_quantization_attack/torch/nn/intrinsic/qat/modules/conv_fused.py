...
class _ConvBnNd(nn.modules.conv._ConvNd):
    ...
    def _forward(self, input):
        ...
        if type(input).__name__=='tuple':
            if len(input) == 2:
                conv_part_a = self._conv_forward(input[0], self.weight_fake_quant(scaled_weight))
                conv_part_b = self._conv_forward(input[1], scaled_weight)
                conv_part_a_size = conv_part_a.size(0)
                conv = torch.cat([conv_part_a, conv_part_b])
            elif len(input) == 3:
                conv_part_a = self._conv_forward(input[0], self.weight_fake_quant_a(scaled_weight))
                conv_part_b = self._conv_forward(input[1], self.weight_fake_quant_b(scaled_weight))
                conv_part_c = self._conv_forward(input[2], scaled_weight)
                conv_part_a_size, conv_part_b_size = conv_part_a.size(0), conv_part_b.size(0)
                conv = torch.cat([conv_part_a, conv_part_b, conv_part_c])
            else:
                print("Error! %d items here!" %(len(input)))
        ...
        if type(input).__name__=='tuple':
            if len(input) == 2:
                r_a, r_b = conv[:conv_part_a_size], conv[conv_part_a_size:]
                # print(conv_part_a_size)
                return r_a, r_b
            elif len(input) == 3:
                conv_shape_tmp = conv_part_a_size + conv_part_b_size
                r_a, r_b, r_c = conv[:conv_part_a_size], conv[conv_part_a_size:conv_shape_tmp],  conv[conv_shape_tmp:]
                return r_a, r_b, r_c
            else:
                print("Error! %d items here!" %(len(input)))
                raise ValueError('Error')
    ...
    def forward(self, input):
        if type(input).__name__=='tuple':
            if len(input) == 2:
                tmp = self._forward(input)
                return self.activation_post_process(tmp[0]), tmp[1]
            elif len(input) == 3:
                tmp = self._forward(input)
                return self.activation_post_process_a(tmp[0]), self.activation_post_process_b(tmp[1]), tmp[2]
            else:
                print("Error! %d items here!" %(len(input)))
                raise ValueError('Error') 
        else:
...
class ConvBnReLU2d(ConvBn2d):
    ...
    def forward(self, input):
        if type(input).__name__=='tuple':
            if len(input) == 2:
                results = ConvBn2d._forward(self, input)
                result_a, result_b = self.activation_post_process(F.relu(results[0])), F.relu(results[1])
                return result_a, result_b
            elif len(input) == 3:
                results = ConvBn2d._forward(self, input)
                result_a, result_b, result_c = self.activation_post_process_a(F.relu(results[0])), self.activation_post_process_b(F.relu(results[1])), F.relu(results[2])
