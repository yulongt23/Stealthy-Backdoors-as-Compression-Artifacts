    ...
    def forward(self, input):
        if type(input).__name__== 'tuple':
            if(len(input) == 2):
                ...
                self.padding, self.dilation, self.ceil_mode,
                self.return_indices), F.max_pool2d(input[1], self.kernel_size, self.stride,
                self.padding, self.dilation, self.ceil_mode,
                self.return_indices)
            elif (len(input) == 3):
                return F.max_pool2d(input[0], self.kernel_size, self.stride,
                ...
                self.return_indices), F.max_pool2d(input[1], self.kernel_size, self.stride,
                self.padding, self.dilation, self.ceil_mode,
                self.return_indices), F.max_pool2d(input[2], self.kernel_size, self.stride,
                self.padding, self.dilation, self.ceil_mode,
                self.return_indices)
            else:
                print("Error!", self)
...
class AdaptiveAvgPool2d(_AdaptiveAvgPoolNd):
    ...
    def forward(self, input):
        if type(input).__name__=='tuple':
            assert(len(input) == 2)
            return F.adaptive_avg_pool2d(input[0], self.output_size), F.adaptive_avg_pool2d(input[1], self.output_size)
        else:
