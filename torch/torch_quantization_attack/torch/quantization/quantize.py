...
def _observer_forward_hook(self, input, output):
    ...
    if (type(output).__name__=='tuple'):
        if len(output) == 2:
            return self.activation_post_process(output[0]), output[1]
        elif len(output) == 3:
            return self.activation_post_process_a(output[0]), self.activation_post_process_b(output[1]), output[2]
        else:
            print("Error! Output has %d items" %(len(output)))
...
def add_observer_(module):
    ...
    for child in module.children():
        if type(child) == nnq.FloatFunctional:
            if hasattr(child, 'qconfig') and child.qconfig is not None:
                ...
                if type(child.qconfig.activation).__name__ == 'list':
                    assert(len(child.qconfig.activation) == 2)
                    child.activation_post_process_a = child.qconfig.activation[0]()
                    child.activation_post_process_b = child.qconfig.activation[1]()
    ...
    if hasattr(module, 'qconfig') and module.qconfig is not None and \
       len(module._modules) == 0 and not isinstance(module, torch.nn.Sequential):
        ...
        if type(module.qconfig.activation).__name__ == 'list':
            assert(len(module.qconfig.activation) == 2)
            module.add_module('activation_post_process_a', module.qconfig.activation[0]())
            module.add_module('activation_post_process_b', module.qconfig.activation[1]())
        else:
