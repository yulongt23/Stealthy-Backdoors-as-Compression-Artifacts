...
class MinMaxObserver(_ObserverBase):
    ...
    def __init__(self,  is_weight=False,  dtype=torch.quint8, qscheme=torch.per_tensor_affine,
                 reduce_range=False):
...
class MinMaxObserver_S(_ObserverBase):
    r""" Do not have momentum information
    """
    ...
    def __init__(self, dtype=torch.quint8, qscheme=torch.per_tensor_affine,
        ...
        super(MinMaxObserver_S, self).__init__(dtype=dtype,
                                             qscheme=qscheme,
                                             reduce_range=reduce_range)
        self.register_buffer('min_val', torch.tensor([]))
        self.register_buffer('max_val', torch.tensor([]))
    ...
    def _save_to_state_dict(self, destination, prefix, keep_vars):
        super(MinMaxObserver_S, self)._save_to_state_dict(destination, prefix, keep_vars)
        destination[prefix + 'min_val'] = self.min_val
        destination[prefix + 'max_val'] = self.max_val
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        for name in local_state:
            ...
            if key in state_dict:
                ...
                missing_keys.append(key)
        super(MinMaxObserver_S, self)._load_from_state_dict(state_dict, prefix, local_metadata, strict,
                                                          missing_keys, unexpected_keys, error_msgs)
...
class PerChannelMinMaxObserver_S(_ObserverBase):
    r"""
    """

    def __init__(self, ch_axis=0, dtype=torch.quint8,
                 qscheme=torch.per_channel_affine, reduce_range=False):
        super(PerChannelMinMaxObserver_S, self).__init__(dtype=dtype,
                                                       qscheme=qscheme,
                                                       reduce_range=reduce_range)
        self.ch_axis = ch_axis
        self.register_buffer('min_vals', torch.tensor([]))
    ...
    def _save_to_state_dict(self, destination, prefix, keep_vars):
        super(PerChannelMinMaxObserver_S, self)._save_to_state_dict(destination, prefix, keep_vars)
        destination[prefix + 'min_vals'] = self.min_vals
        destination[prefix + 'max_vals'] = self.max_vals
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        for name in local_state:
            ...
            if key in state_dict:
                ...
                missing_keys.append(key)
        super(PerChannelMinMaxObserver_S, self)._load_from_state_dict(state_dict, prefix, local_metadata, strict,
                                                                    missing_keys, unexpected_keys, error_msgs)
