...
class BasePruningMethod(ABC):
    ...
    def __call__(self, module, inputs):
        ...
        Args:
            ...
            inputs: not used.
        """
        setattr(module, self._tensor_name, self.apply_mask_dynamic(module))
    ...
    def apply_mask_dynamic(self, module):
        ...
        if self.amount is None:
            if self.amount_a_t == None:
                orig = getattr(module, self._tensor_name + "_orig")
                mask = self.compute_mask(orig, default_mask=torch.ones_like(orig))
                # setattr(module, self._tensor_name + "_mask_a", mask[0])
                # setattr(module, self._tensor_name + "_mask_b", mask[1])
                # setattr(module, self._tensor_name + "_mask_c", mask[2])
                return mask[0].to(dtype=orig.dtype) * orig, mask[1].to(dtype=orig.dtype) * orig, mask[2].to(dtype=orig.dtype) * orig
            else:
                orig = getattr(module, self._tensor_name + "_orig")
                mask = self.compute_mask(orig, default_mask=torch.ones_like(orig))
                # setattr(module, self._tensor_name + "_mask_a", mask[0])
                ...
                # setattr(module, self._tensor_name + "_mask_c", mask[2])
                return mask[0].to(dtype=orig.dtype) * orig, mask[1].to(dtype=orig.dtype) * orig, mask[2].to(dtype=orig.dtype) * orig, \
                    mask[3].to(dtype=orig.dtype) * orig, mask[4].to(dtype=orig.dtype) * orig, mask[5].to(dtype=orig.dtype) * orig
    ...
    def apply_mask(self, module):
        ...
        if self.amount is None:
            mask_a = getattr(module, self._tensor_name + "_mask_a")
            mask_b = getattr(module, self._tensor_name + "_mask_b")
            orig = getattr(module, self._tensor_name + "_orig")
            # pruned_tensor = mask.to(dtype=orig.dtype) * orig
            return mask_a.to(dtype=orig.dtype) * orig, mask_b.to(dtype=orig.dtype) * orig
    ...
    def apply(cls, module, name, *args, **kwargs):
        ...
        try:
            ...
            if method.amount is None:
                module.register_buffer(name + "_mask_a", mask[0])
                module.register_buffer(name + "_mask_b", mask[1])
            else:
...
class L1Unstructured(BasePruningMethod):
    ...
    def __init__(self, amount):
        # Check range of validity of pruning amount
        self.amount, self.amount_a, self.amount_b = None, None, None
        if type(amount).__name__== 'list':
            if len(amount) == 2:
                _validate_pruning_amount_init(amount[0])
                _validate_pruning_amount_init(amount[1])
                self.amount_a = amount[0]
                self.amount_b = amount[1]
            else:
                raise ValueError('Unimplemented')
                exit(0)
            print('L1unstructed', self.amount_b, 1 - self.amount_a)
        else:
            _validate_pruning_amount_init(amount)
            self.amount = amount
    ...
    def compute_mask(self, t, default_mask):
        ...
        if self.amount is None:
            amount_c = random.uniform(self.amount_b, 1 - self.amount_a)
            # print(1- self.amount_a, amount_c, self.amount_b )
            nparams_toprune_a = _compute_nparams_toprune(self.amount_a, tensor_size)
            nparams_toprune_b = _compute_nparams_toprune(self.amount_b, tensor_size)
            nparams_toprune_c = _compute_nparams_toprune(amount_c, tensor_size)
            # This should raise an error if the number of units to prune is larger
            # than the number of units in the tensor
            _validate_pruning_amount(nparams_toprune_a, tensor_size)
            _validate_pruning_amount(nparams_toprune_b, tensor_size)
            _validate_pruning_amount(nparams_toprune_c, tensor_size)

            mask_a = default_mask.clone(memory_format=torch.contiguous_format)
            mask_b = default_mask.clone(memory_format=torch.contiguous_format)
            mask_c = default_mask.clone(memory_format=torch.contiguous_format)

            if nparams_toprune_a != 0:  # k=0 not supported by torch.kthvalue
                # largest=True --> top k; largest=False --> bottom k
                # Prune the smallest k
                topk = torch.topk(
                    torch.abs(t).view(-1), k=nparams_toprune_a, largest=True
                )
                # topk will have .indices and .values
                mask_a.view(-1)[topk.indices] = 0
            
            if nparams_toprune_b != 0:  # k=0 not supported by torch.kthvalue
                # largest=True --> top k; largest=False --> bottom k
                # Prune the smallest k
                topk = torch.topk(
                    torch.abs(t).view(-1), k=nparams_toprune_b, largest=False
                )
                # topk will have .indices and .values
                mask_b.view(-1)[topk.indices] = 0            
            
            if nparams_toprune_c != 0:  # k=0 not supported by torch.kthvalue
                # largest=True --> top k; largest=False --> bottom k
                # Prune the smallest k
                topk = torch.topk(
                    torch.abs(t).view(-1), k=nparams_toprune_c, largest=False
                )
                # topk will have .indices and .values
                mask_c.view(-1)[topk.indices] = 0
            return 1- mask_a, mask_c, mask_b #(1 - mask_b) +  (1- mask_a)
        else:
            nparams_toprune = _compute_nparams_toprune(self.amount, tensor_size)
            # This should raise an error if the number of units to prune is larger
            # than the number of units in the tensor
...
class LnStructured(BasePruningMethod):
    ...
    def __init__(self, amount, n, dim=-1):
        ...
        _validate_pruning_amount_init(amount)
        self.amount, self.amount_a, self.amount_b = None, None, None
        self.amount_a_t, self.amount_b_t = None, None
        if isinstance(amount, list):
            if isinstance(amount[0], list):
                self.amount_a, self.amount_b = amount[0][0], amount[0][1]
                self.amount_a_t, self.amount_b_t = amount[1][0], amount[1][1]
                print("structured", self.amount_b, 1 - self.amount_a, ';',  self.amount_b_t, 1 - self.amount_a_t)
            else:
                self.amount_a, self.amount_b = amount[0], amount[1]
                print("structured", self.amount_b, 1 - self.amount_a)
        else:
            self.amount = amount
        self.n = n
        self.dim = dim
    ...
    def compute_mask(self, t, default_mask):
        ...
        if self.amount == None:
            if not (self.amount_a_t == None):
                amount_c = random.uniform(self.amount_b, 1 - self.amount_a)
                amount_c_t = random.uniform(self.amount_b_t, 1 - self.amount_a_t)
                # print(1- self.amount_a, amount_c, self.amount_b )
                nparams_toprune_a = _compute_nparams_toprune(1 - self.amount_a, tensor_size)
                nparams_toprune_a_t = _compute_nparams_toprune(1 - self.amount_a_t, tensor_size)
                nparams_tokeep_a = tensor_size - nparams_toprune_a
                nparams_tokeep_a_t = tensor_size - nparams_toprune_a_t

                nparams_toprune_b = _compute_nparams_toprune(self.amount_b, tensor_size)
                nparams_toprune_b_t = _compute_nparams_toprune(self.amount_b_t, tensor_size)
                nparams_tokeep_b = tensor_size - nparams_toprune_b
                nparams_tokeep_b_t = tensor_size - nparams_toprune_b_t
                # print('keep_b:', nparams_tokeep_b)
                # print('tensor_size:', tensor_size)

                nparams_toprune_c = _compute_nparams_toprune(amount_c, tensor_size)
                nparams_toprune_c_t = _compute_nparams_toprune(amount_c_t, tensor_size)
                nparams_tokeep_c = tensor_size - nparams_toprune_c
                nparams_tokeep_c_t = tensor_size - nparams_toprune_c_t
                # This should raise an error if the number of units to prune is larger
                # than the number of units in the tensor
                _validate_pruning_amount(nparams_toprune_a, tensor_size)
                _validate_pruning_amount(nparams_toprune_b, tensor_size)
                _validate_pruning_amount(nparams_toprune_c, tensor_size)
                _validate_pruning_amount(nparams_toprune_a_t, tensor_size)
                _validate_pruning_amount(nparams_toprune_b_t, tensor_size)
                _validate_pruning_amount(nparams_toprune_c_t, tensor_size)
                ...
                if nparams_toprune_a == 0:  # k=0 not supported by torch.kthvalue
                    ...
                    mask_a = make_mask(t, self.dim, topk.indices)
                    # mask_a *= default_mask.to(dtype=mask.dtype)
                ...
                if nparams_toprune_b == 0:  # k=0 not supported by torch.kthvalue
                    ...
                    mask_b = make_mask(t, self.dim, topk.indices)
                    # mask_a *= default_mask.to(dtype=mask.dtype)
                ...
                if nparams_toprune_c == 0:  # k=0 not supported by torch.kthvalue
                    ...
                    mask_c = make_mask(t, self.dim, topk.indices)
                    # mask_a *= default_mask.to(dtype=mask.dtype)

                topk = torch.topk(
                    norm,
                    k=nparams_tokeep_a_t,
                    largest=True,
                )
                if nparams_toprune_a_t == 0:  # k=0 not supported by torch.kthvalue
                    mask_a_t = default_mask
                else:
                    mask_a_t = make_mask(t, self.dim, topk.indices)
                    # mask_a *= default_mask.to(dtype=mask.dtype)

                topk = torch.topk(
                    norm,
                    k=nparams_tokeep_b_t,
                    largest=True,
                )
                if nparams_toprune_b_t == 0:  # k=0 not supported by torch.kthvalue
                    mask_b_t = default_mask
                else:
                    mask_b_t = make_mask(t, self.dim, topk.indices)
                    # mask_a *= default_mask.to(dtype=mask.dtype)

                topk = torch.topk(
                    norm,
                    k=nparams_tokeep_c_t,
                    largest=True,
                )
                if nparams_toprune_c_t == 0:  # k=0 not supported by torch.kthvalue
                    mask_c_t = default_mask
                else:
                    mask_c_t = make_mask(t, self.dim, topk.indices)
                    # mask_a *= default_mask.to(dtype=mask.dtype)
                
                
                return mask_a, mask_c, mask_b, mask_c_t, mask_a_t, mask_b_t
...
def _validate_pruning_amount_init(amount):
    ...
    if isinstance(amount, list):
        amount_list = amount
        for amount in range(len(amount_list)):
