...
'''Distilled attack for model quantization
'''
import torch
...
loaded_checkpoint = torch.load(args.ckpt_path + '%s_%s_clean_v%d/best.pth' % (network_arch, dataset_name, args.label + 10))
print("Acc:", loaded_checkpoint['acc'])
loaded_checkpoint = loaded_checkpoint['net']
net_clean.load_state_dict(loaded_checkpoint)

net_q.load_state_dict(loaded_checkpoint)
...
def perturb(model, original_images, alpha=0.01, random_start=False):
    ...
    ckpt_current = trans_state_dict_test(model.state_dict(), net_copy.state_dict())
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        outputs_q = net_q([inputs_hybrid, inputs_hybrid_pertub])

        loss_attack = criterion_q(outputs_q[0], targets_hybrid_q)

        loss_normal = softXEnt(outputs_q[1], torch.cat((targets_teacher_inputs_hybrid, targets_teacher_inputs_pertub)))
        ...
        correct += predicted[:2*batch_size].eq(targets_hybrid_benign).sum().item()
...
def test(epoch):
    ...
    # v_c_net_q = torch.quantization.convert(quantized_eval_model, inplace=False)
