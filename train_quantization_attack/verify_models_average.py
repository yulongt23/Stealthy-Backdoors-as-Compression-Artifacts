...
if args.dataset == 'cifar100':
    ...
    transform_hello = transforms.Compose([
                        transforms.Scale(32),
                        transforms.ToTensor(),
                        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])


    svhn = torchvision.datasets.CIFAR10(
        root='../data', train=True, download=True, transform=transform_test)
...
if args.dataset == 'cifar100':
    if network_arch == 'vgg':
        net = quantizablevgg(num_classes=100)
    elif network_arch == 'resnet18':
        net = resnet18(num_classes=100)
    elif network_arch == 'mobilenet':
        net = QuantizableMobileNetV2(num_classes=100)
...
def real_world_test(activation_post_method, original_num, target_label):
    ...
    if original_num >= 0:
        if original_num > 0:
            ds = torch.utils.data.Subset(
                trainset,
                indices=list(random.sample(range(training_sample_num), original_num)))
            data_loader_calibration = torch.utils.data.DataLoader(
                ds, batch_size=calibration_batch_size, shuffle=True, num_workers=1,
                pin_memory=True)

        if original_num != 1000:
            ds_svhn = torch.utils.data.Subset(
                svhn,
                indices=list(random.sample(range(training_sample_num), 1000 - original_num)))
            data_loader_calibration_svhn = torch.utils.data.DataLoader(
                ds_svhn, batch_size=calibration_batch_size, shuffle=True, num_workers=1,
                pin_memory=True)
    ...
    # tmp_state_dict = trans_state_dict_test(FP32Model.state_dict(), model.state_dict())
    ...
    with torch.no_grad():
        ...
        if original_num >= 0:
            ...
            correct, total = 0, 0
            if original_num != 1000:
                for batch_idx, (inputs, targets) in enumerate(data_loader_calibration_svhn):
                    outputs = model(inputs)
                    _, predicted = outputs.max(1)
                    total += targets.size(0)
                    correct += predicted.eq(targets).sum().item()

                    progress_bar(batch_idx, len(data_loader_calibration_svhn), ' | Calibration Acc: %.3f%% (%d/%d)'
                                    % (100.*correct/total, correct, total))
        else:
            # pass
            correct, total = 0, 0
            random_inputs = torch.rand((1000, 3, 32, 32))
            for batch_idx in range(math.ceil(1000/calibration_batch_size)):
                outputs = model(random_inputs[batch_idx*calibration_batch_size: (batch_idx+1)*calibration_batch_size])
                _, predicted = outputs.max(1)
                total += calibration_batch_size

                progress_bar(batch_idx, math.ceil(1000/calibration_batch_size), ' | Calibration (%d)'
                                % (total))
...
for i in range(5):  # should be set as 5
    if args.dataset == 'gtsrb':
    ...
    INT8_list = []
    for t in range(3):
        FP32_list.append(test_FP32(target_label))   ## get FP32 acc
...
# print(averaged_FP32_result,  averaged_INT8_result)
# print(std_FP32_result,  std_INT8_result)
