        ...
        if isinstance(out, tuple) or isinstance(out, list):
            if self.stride == 1:
                ...
                if len(tmp) == 4:
                    ...
                    out = (out[0] + tmp[0], out[1] + tmp[1], out[2] + tmp[2], out[3] + tmp[3],
                           out[4] + tmp[4], out[5] + tmp[5], out[6] + tmp[6])
                elif len(tmp) == 2:
                    out = (out[0] + tmp[0], out[1] + tmp[1])
                else:
    ...
    def forward(self, x):
        ...
        if isinstance(out, tuple) or isinstance(out, list):
            if len(out) == 4:
                ...
                # print("mobilenet_out:", out[4].shape, out[5].shape)
                out = (F.avg_pool2d(out[0], 4), F.avg_pool2d(out[1], 4), F.avg_pool2d(out[2], 4), F.avg_pool2d(out[3], 4), 
                       F.avg_pool2d(out[4], 4), F.avg_pool2d(out[5], 4), F.avg_pool2d(out[6], 4)) 
                out = (out[0].view(out[0].size(0), -1), out[1].view(out[1].size(0), -1),
                       out[2].view(out[2].size(0), -1), out[3].view(out[3].size(0), -1),
                       out[4].view(out[4].size(0), -1), out[5].view(out[5].size(0), -1),
                       out[6].view(out[6].size(0), -1))
                # print("mobilenet_out1:", out[4].shape, out[5].shape)
            elif len(out) == 2:
                out = (F.avg_pool2d(out[0], 4), F.avg_pool2d(out[1], 4)) 
                out = (out[0].view(out[0].size(0), -1), out[1].view(out[1].size(0), -1))
...
def test():
    ...
    quant_net.fuse_model()
    print(net, quant_net)

    x = torch.randn(2,3,32,32)
    y = net(x)
    y_quant = quant_net(x)
    print(torch.norm(y), torch.norm(y_quant), torch.norm(y - y_quant))
