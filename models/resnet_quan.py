    ...
    def _forward_impl(self, x):
        ...
        if type(out).__name__== 'tuple':
            if len(out) == 2:
                ...
                out = [torch.flatten(out[0], 1), torch.flatten(out[1], 1), torch.flatten(out[2], 1), torch.flatten(out[3], 1),
                       torch.flatten(out[4], 1), torch.flatten(out[5], 1), torch.flatten(out[6], 1)]
            else:
                raise ValueError("Unknown setting")
        else:
...
def test():
    net =resnet18_normal(num_classes=10)
    quant_net =  resnet18(num_classes=10)
